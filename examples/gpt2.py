...
# Buton kontrolü için GPIO ayarları
GPIO.setmode(GPIO.BCM)
GPIO.setup(BUTTON_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)

# Buton basıldığında çağrılacak fonksiyon
def start_game(channel):
    global card_read, start_time
    if not card_read:
        card_read = True
        print("Butona basıldı! İyi oyunlar!")
        start_time = pygame.time.get_ticks() / 1000  # Başlangıç zamanını al

# Butonun kesme (interrupt) işlemi tanımlanıyor
GPIO.add_event_detect(BUTTON_PIN, GPIO.FALLING, callback=start_game, bouncetime=300)
